...
This document provides comprehensive testing instructions for all endpoints in the Mallu Magic backend API.

## ⚠️ IMPORTANT: Authentication Setup First!

**Before testing any endpoints, you MUST set up authentication in Postman!**

👉 **See [POSTMAN_SETUP.md](./POSTMAN_SETUP.md) for detailed step-by-step instructions on:**
- Setting up Postman environment
- Configuring automatic token saving
- Setting up Authorization headers
- Troubleshooting "Unauthorized" errors

**Quick Setup:**
1. Create Postman environment with `base_url` and `token` variables
2. Add Tests script to login request to auto-save token
3. Use Bearer Token authorization with `{{token}}` for all requests
...
- `token`: Will be set automatically after login (see POSTMAN_SETUP.md)
- `refresh_token`: Will be set automatically after login

### Pre-requisites
1. MongoDB Atlas connection string configured
2. Environment variables set in Vercel/Postman
3. At least one admin user created in the database (run `npm run seed`)
...
### 401 Unauthorized
- Check if token is set in headers/cookies
...
- Verify data types match (ObjectId, dates, etc.)

### 404 Not Found
- Verify ID format is correct (MongoDB ObjectId)
...
1. **Create Environment:**
   ...
   - `token`: JWT token (auto-set from login)
   - `refresh_token`: Refresh token (auto-set from login)
...
- All dates should be in ISO 8601 format: `2024-12-31T00:00:00Z`
- All MongoDB ObjectIds are 24-character hex strings
- Pagination defaults: page=1, limit=50
- All endpoints return JSON with `success`, `data`, and optional `error` fields
- Audit logs are automatically created for all write operations
- Stock ledger entries are automatically created for all stock movements
